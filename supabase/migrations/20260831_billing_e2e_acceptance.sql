create table if not exists public.billing_e2e_acceptance_runs (
  ...
  deployment_ref text not null check (length(trim(deployment_ref)) >= 7),
  ...
  unique(environment,workspace_id,deployment_ref)
);
...
comment on table public.billing_e2e_acceptance_runs is 'Server-owned Stripe sandbox billing journey acceptance ledger, scoped to one deployed release. Browser roles have no direct access.';
