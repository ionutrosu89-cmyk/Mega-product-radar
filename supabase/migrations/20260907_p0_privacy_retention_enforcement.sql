-- Mirrors the production privacy-retention control so fresh environments cannot drift.
-- Public Free anonymous demand telemetry: 90 days.
-- Account usage/journey/feedback beta telemetry: 12 months.

create schema if not exists private;

create or replace function private.enforce_privacy_retention_v1()
returns table(
  deleted_free_demand bigint,
  deleted_usage bigint,
  deleted_feedback bigint,
  deleted_journey bigint,
  deleted_beta_feedback bigint
)
language plpgsql
security definer
...
begin
  delete from public.free_demand_events
...
revoke all on function private.enforce_privacy_retention_v1() from public, anon, authenticated, service_role;
grant execute on function private.enforce_privacy_retention_v1() to postgres;
