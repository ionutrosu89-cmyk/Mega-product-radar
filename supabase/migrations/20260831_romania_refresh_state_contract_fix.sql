...
begin
  ...
  if outcome='COMPLETED' then
    ...
    update public.refresh_queue set state='DONE',completed_at=now(),lease_owner=null,lease_expires_at=null,last_error=null where id=p_job_id;
  ...
  return jsonb_build_object('ok',true,'jobId',p_job_id,'outcome',outcome,'surface',q.target_surface,'queueState',case when outcome='COMPLETED' then 'DONE' else 'PENDING' end,'observationId',inserted_observation_id,'paidCallsTriggered',0,'providerSpendEur',0,'purchaseAuthorized',false,'salesEvidenceClass','NOT_VERIFIED_SALES');
end;$function$;