-- Select deterministic Amazon exact-ASIN products that still need longitudinal history.
-- Read-only target selection; no provider calls, no identity mutation.
...
as $function$
  ...
  from candidates c
  order by c.existing_observation_count asc,c.external_id asc
