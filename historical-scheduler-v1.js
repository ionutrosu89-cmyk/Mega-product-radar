...
function firstObservationAtOrAfterWindow(rows=[],windowHours){
  if(rows.length<2)return null;
  const anchorMs=Date.parse(rows[0].observedAt),threshold=anchorMs+windowHours*3600000;
  return rows.slice(1).find(row=>Date.parse(row.observedAt)>=threshold)||null;
}
...
export function buildHistoricalSchedule(history=[],{now=new Date().toISOString(),windows=HISTORICAL_WINDOWS_V1}={}){
  ...
  for(const {scopeKey,rows} of normalizedSeries(history)){
    const anchor=rows[0],latest=rows.at(-1),anchorMs=Date.parse(anchor.observedAt);
    for(const window of windows){
      const dueAt=new Date(anchorMs+window.hours*3600000).toISOString();
      const satisfiedBy=firstObservationAtOrAfterWindow(rows,window.hours);
      ...
      items.push(Object.freeze({
        ...
        window:window.key,windowHours:window.hours,priority:window.priority,anchorObservedAt:anchor.observedAt,lastObservedAt:latest.observedAt,dueAt,status:satisfiedBy?'WINDOW_SATISFIED':due?'DUE':'WAITING',
  ...
  return Object.freeze({
    ...
    dueItems:Object.freeze(dueItems),items:Object.freeze(items),policy:'SAME_PLATFORM_EXTERNAL_ID_SURFACE_ONLY; WINDOWS_ANCHORED_TO_FIRST_OBSERVATION; 24H_7D_30D_90D_MILESTONES; SCHEDULE_DOES_NOT_EXECUTE_PROVIDER_CALLS; TITLE_NEVER_BINDS_HISTORY',
