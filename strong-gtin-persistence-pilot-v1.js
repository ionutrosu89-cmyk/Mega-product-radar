...
export function buildStrongGtinPersistencePilot(bundle={},options={}){
  ...
  const gtinKeys=new Set((bundle.identities||[]).filter(x=>nsOf(x)==='GTIN'&&String(x.valueNorm||x.value_norm||'')).map(keyOf));
  ...
  const strongBundle={
    ...bundle,
    products,
    identities:(bundle.identities||[]).filter(x=>allowed.has(keyOf(x))&&nsOf(x)==='GTIN'),
...
export function validateStrongGtinPersistencePilot(batch={}){
  ...
  return{schema:'MPR_STRONG_GTIN_PERSISTENCE_PILOT_VALIDATION_V1',valid:reasons.length===0,reasons};
}
