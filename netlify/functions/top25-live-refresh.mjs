...
export function createTop25LiveRefreshHandler({env=process.env,fetchImpl=fetch,now=()=>new Date(),logger=console}={}){
  return async ()=>{
    ...
    const summary={checkedAt:now().toISOString(),providers:[{provider:'EBAY',access,status:'SKIPPED',published:0}],purchaseAuthorized:false};
    if(access==='READY_TO_COLLECT'){
      ...
      if(!secret)summary.providers[0].status='INTERNAL_SECRET_REQUIRED';
        ...
        const handler=createEbayCrossMarketRefreshHandler({env,fetchImpl,now});
