...
export const REQUIRED_CHECKS=Object.freeze([
  'LEGAL_OPERATOR','LEGAL_TERMS','PRIVACY_GDPR','SUPPORT_REFUNDS','DOMAIN_BRAND','EXTERNAL_BETA','INCIDENT_ROLLBACK'
]);
...
export function createLaunchReadinessHandler({fetch:fetchImpl=fetch,env=process.env}={}){return async request=>{try{
  ...
  if(request.method==='POST'){
    const body=await request.json().catch(()=>({}));const checkCode=String(body.checkCode||'').toUpperCase(),status=String(body.status||'').toUpperCase(),note=String(body.evidenceNote||'').trim().slice(0,2000);
    if(!REQUIRED_CHECKS.includes(checkCode))return Response.json({ok:false,error:'Unknown launch check'},{status:400});if(!STATUS.has(status))return Response.json({ok:false,error:'Invalid launch status'},{status:400});if(status==='PASS'&&note.length<8)return Response.json({ok:false,error:'Evidence note required before PASS'},{status:400});
