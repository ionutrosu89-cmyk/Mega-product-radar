...
function aggregateFreeDemand(events=[]){
  ...
  const sessionsWhere=predicate=>new Set(events.filter(predicate).map(row=>row.page_session_id).filter(Boolean));
  ...
  const productSessions=sessionsWhere(row=>row.event_name==='FREE_SOURCE_OPENED'||(row.event_name==='FREE_PRODUCT_OPENED'&&metadata(row).target!=='COMPARE'));
  const decisions=sessionsWhere(row=>row.event_name==='FREE_DECISION_REACHED'&&['INVESTIGATE','HOLD','UNKNOWN'].includes(String(metadata(row).decision||'')));
  const shortlist=sessionsWhere(row=>row.event_name==='FREE_DECISION_REACHED'&&metadata(row).decision==='SHORTLIST_ADD');
  const comparisons=sessionsWhere(row=>row.event_name==='FREE_PRODUCT_OPENED'&&metadata(row).target==='COMPARE');
  const platformRequests=sessionsWhere(row=>row.event_name==='FREE_DECISION_REACHED'&&metadata(row).decision==='REQUEST_PLATFORM');
  const platformSelections=sessionsWhere(row=>row.event_name==='FREE_NICHE_SELECTED'&&String(metadata(row).target||'').startsWith('PLATFORM:'));
  ...
  for(const row of events){
    if(row.event_name!=='FREE_DECISION_REACHED'||metadata(row).decision!=='REQUEST_PLATFORM')continue;
    const label=String(metadata(row).target||'').replace(/^PLATFORM:/,'').trim();
    if(label)requestedPlatforms[label]=(requestedPlatforms[label]||0)+1;
  }
  return {
    totals:{events:events.length,sessions:allSessions.size,landingSessions:landing.size,top25ClickSessions:top25Clicks.size,top25ViewSessions:top25Views.size,nicheSessions:nicheSessions.size,platformSelectionSessions:platformSelections.size,productSessions:productSessions.size,decisionSessions:decisions.size,shortlistSessions:shortlist.size,comparisonSessions:comparisons.size,platformRequestSessions:platformRequests.size,signupSessions:signup.size},
    conversion:{landingToTop25:pct(top25Clicks.size,landing.size),nicheFromTop25:pct(nicheSessions.size,top25Views.size),productFromTop25:pct(productSessions.size,top25Views.size),decisionFromTop25:pct(decisions.size,top25Views.size),shortlistFromTop25:pct(shortlist.size,top25Views.size),compareFromTop25:pct(comparisons.size,top25Views.size),platformRequestFromTop25:pct(platformRequests.size,top25Views.size),signupFromTop25:pct(signup.size,top25Views.size)},
    topNiches:countBy('niche_id').slice(0,10),
    requestedPlatforms:Object.entries(requestedPlatforms).sort((a,b)=>b[1]-a[1]).slice(0,10).map(([label,count])=>({label,count})),
