...
function aggregate({events=[],workspaces=[],preferences=[],subscriptions=[]},days){
  ...
  const checkoutStarted=wsWith('CHECKOUT_STARTED'),checkoutCompleted=wsWith('CHECKOUT_COMPLETED');
  const subscriptionActivated=wsWith('SUBSCRIPTION_ACTIVATED'),planChanged=wsWith('PLAN_CHANGED');
  const cancelScheduled=wsWith('SUBSCRIPTION_CANCEL_SCHEDULED'),cancelUnscheduled=wsWith('SUBSCRIPTION_CANCEL_UNSCHEDULED'),ended=wsWith('SUBSCRIPTION_ENDED');
  ...
  const activeWs=unique(events,'workspace_id'),activeUsers=unique(events,'user_id');
  const eventCounts={};for(const e of events)eventCounts[e.event_name]=(eventCounts[e.event_name]||0)+1;
  const funnel=[
    {key:'onboarding_view',label:'Onboarding văzut',workspaces:onboardingView.size},{key:'onboarding_complete',label:'Onboarding completat',workspaces:onboardingComplete.size},{key:'home',label:'Home activat',workspaces:home.size},{key:'discover',label:'Discover folosit',workspaces:discover.size},{key:'radar',label:'Radar folosit',workspaces:radar.size},{key:'launch',label:'Launch folosit',workspaces:launch.size},{key:'checkout_started',label:'Checkout pornit',workspaces:checkoutStarted.size},{key:'checkout_completed',label:'Checkout finalizat',workspaces:checkoutCompleted.size},{key:'paid',label:'Abonament activ',workspaces:activePaid.size}
  ];
  for(let i=0;i<funnel.length;i++)funnel[i].conversionFromPrevious=i===0?100:pct(funnel[i].workspaces,funnel[i-1].workspaces);
  const retained=Math.max(0,activePaid.size-cancelPending.size);
  return {days,generatedAt:new Date().toISOString(),dataScope:'REAL_EVENT_DATA',totals:{workspaces:workspaces.length,activeWorkspaces:activeWs.size,activeUsers:activeUsers.size,events:events.length,onboardingCompleted:onboardingComplete.size,upgradeIntentWorkspaces:upgradeIntent.size,checkoutStartedWorkspaces:checkoutStarted.size,checkoutCompletedWorkspaces:checkoutCompleted.size,subscriptionActivatedWorkspaces:subscriptionActivated.size,planChangedWorkspaces:planChanged.size,activePaidWorkspaces:activePaid.size,cancelPendingWorkspaces:cancelPending.size,cancelScheduledWorkspaces:cancelScheduled.size,cancelUnscheduledWorkspaces:cancelUnscheduled.size,endedWorkspaces:ended.size,retainedPaidWorkspaces:retained},byPlan,funnel,eventCounts,retention:{activePaid:activePaid.size,cancelPending:cancelPending.size,retainedPaid:retained,endedInWindow:ended.size,retentionRate:pct(retained,activePaid.size),cancelPendingRate:pct(cancelPending.size,activePaid.size),churnRate:pct(ended.size,Math.max(1,subscriptionActivated.size))},conversion:{onboarding:pct(onboardingComplete.size,onboardingView.size),activation:pct(home.size,onboardingComplete.size),discoverToRadar:pct(radar.size,discover.size),radarToLaunch:pct(launch.size,radar.size),upgradeIntentFromActive:pct(upgradeIntent.size,activeWs.size),checkoutFromUpgrade:pct(checkoutStarted.size,upgradeIntent.size),checkoutCompletion:pct(checkoutCompleted.size,checkoutStarted.size),paidFromCheckout:pct(activePaid.size,checkoutCompleted.size),paidFromActive:pct(activePaid.size,activeWs.size)}};
}
