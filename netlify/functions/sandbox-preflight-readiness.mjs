...
export function createSandboxPreflightReadinessHandler({fetch:fetchImpl=fetch,env=process.env,now=()=>new Date()}={}){
  return async request=>{try{
    ...
    const workspaceFilter=configuredId?`id=eq.${encodeURIComponent(configuredId)}`:`slug=eq.${encodeURIComponent(workspaceSlug)}`;
    const workspaceResult=await jsonFetch(`${supabaseUrl}/rest/v1/workspaces?select=id,plan&${workspaceFilter}&limit=1`,{headers:dbHeaders},fetchImpl);
