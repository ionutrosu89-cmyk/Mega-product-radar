...
function updateStatus(data, source) { document.querySelector('#dataSource').textContent=source; document.querySelector('#lastScan').textContent=data.updatedAt?new Date(data.updatedAt).toLocaleString('ro-RO'):'Nicio scanare'; const status=data.scan?.status || (data.live?'completed':'idle');document.querySelector('#scanStatus').textContent=status;document.querySelector('#scanError').textContent=data.scan?.error?` — ${data.scan.error}`:'';document.querySelector('#runScan').disabled=['queued','running'].includes(status); }

export async function loadRadar(fetcher=fetch) {
  try { const response=await fetcher('/api/radar/data',{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const data=await response.json();if(data.live&&normalizeProducts(data.products).length){setProducts(data.products);updateStatus(data,'LIVE • radar-data');return data;}updateStatus(data,'Fallback • products.json'); }
  catch(error){updateStatus({scan:{status:'error',error:error.message}},'Fallback • products.json');}
  const fallback=await fetcher(`products.json?t=${Date.now()}`,{cache:'no-store'});if(!fallback.ok)throw new Error(`Fallback HTTP ${fallback.status}`);setProducts(await fallback.json());return {live:false,products};
}
export async function runScan(fetcher=fetch){const button=document.querySelector('#runScan');button.disabled=true;document.querySelector('#scanStatus').textContent='queued';document.querySelector('#scanError').textContent='';try{const response=await fetcher('/api/radar/trigger',{method:'POST'});const data=await response.json();if(!response.ok)throw new Error(data.error||`HTTP ${response.status}`);updateStatus({scan:data.scan},document.querySelector('#dataSource').textContent);setTimeout(()=>loadRadar(fetcher),3000);return data;}catch(error){updateStatus({scan:{status:'error',error:error.message}},document.querySelector('#dataSource').textContent);throw error;}finally{button.disabled=false;}}

if(typeof document!=='undefined'){document.querySelector('#search').addEventListener('input',render);document.querySelector('#category').addEventListener('change',render);document.querySelector('#runScan').addEventListener('click',()=>runScan().catch(()=>{}));loadRadar().catch(error=>updateStatus({scan:{status:'error',error:error.message}},'Indisponibil'));}
