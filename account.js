...
import {localCloudSummary,pushLocalToCloud,pullCloudToLocal} from './cloud-sync.js';
...
$('#cloudPush').addEventListener('click',async()=>{
  if(!confirm('Trimiti datele locale curente in cloud? Copia cloud pentru cele 6 categorii va fi inlocuita.'))return;
...
$('#cloudPull').addEventListener('click',async()=>{
  if(!confirm('Descarci copia cloud? Datele locale din cele 6 categorii vor fi inlocuite.'))return;
