...
export function evaluateCommercialDecision(p={},state={}){
  ...
  const scoreInputs=deriveCommercialScoreInputs({product:p,decision:{gates},quantityEconomics});
  if(scoreInputs.economicsScore===null&&economics?.priceComplete)scoreInputs.economicsScore=Math.max(0,Math.min(100,(n(economics.margin)-10)*2+(n(economics.roi)-20)*.45));
  if(scoreInputs.logisticsScore===null&&landed)scoreInputs.logisticsScore=80;
  if(scoreInputs.capitalEfficiencyScore===null&&testReady&&landed)scoreInputs.capitalEfficiencyScore=Math.max(0,Math.min(100,100-Math.log10(Math.max(1,quantity*n(landed.landedPerUnit)))*18));
  const commercialScore=commercialScoreV1(scoreInputs);
  const quantityRecommendation=quantityEconomics?.recommendation||null;
  return {version:'3.2-commercial-score',status,commercialAction,verdict,quantity,gates,gateCount:9,passedGates:Object.values(gates).filter(Boolean).length,blockers,commercialReadiness:Math.round(Object.values(gates).filter(Boolean).length/9*100),commercialScore,quantityRecommendation,quantityEconomicsStatus:quantityEconomics?.status||'UNKNOWN',unitLandedCost:landed?.landedPerUnit??null,landedCostConfirmed:Boolean(landed),targetSalePrice:sell||null,testBudget:testReady?quantity*n(landed?.landedPerUnit):null,expectedRevenue:testReady?quantity*sell:null,expectedGrossProfit:testReady?quantity*n(economics?.profit):null,confidenceScore:n(p?.dataConfidence?.overall),economics:economics?{profit:economics.profit,margin:economics.margin,roi:economics.roi,breakEvenSell:economics.breakEvenSell}:null,estimationEvidence:{salesEstimateStatus:salesModel.status||'UNKNOWN',estimatedUnits30d:salesModel.estimatedUnits30d??null,salesEstimateConfidence:salesModel.confidence??null,actualCompetitorSalesObserved:actualSalesObserved},buyGate:{completedRealTest:feedback?.passed===true,source:feedback?.source??null,testOutcome:feedback?.testOutcome??null,sellThroughPct:feedback?.sellThroughPct??null,actualMarginPct:feedback?.actualMarginPct??null,actualUnitProfitRon:feedback?.actualUnitProfitRon??null,returnRatePct:feedback?.returnRatePct??null,measuredAt:feedback?.measuredAt??null},nextAction:buyReady?'Planifică reaprovizionarea controlată folosind rezultatele reale ale testului.':blockers[0]||'Comandă lotul de test și înregistrează rezultatul real în Test Execution.',policy:'Private money gate: TEST requires verified supplier + confirmed landed cost + healthy economics. Commercial Score and quantity optimization are derived ranking aids only and can never override a hard gate. BUY additionally requires the latest measured real test to pass.'};
}
