...
test('foreign-currency quote requires explicit FX rate and source',()=>{
  const base={supplierPackageVerified:true,romaniaGapExact:true,quoteCurrency:'USD',sellableSets:100,goodsValueRon:5000,internationalFreightRon:1000,insuranceRon:50,dutyRate:.12,importVatRate:.21,brokerageRon:100,destinationHandlingRon:100,domesticTransportRon:100,complianceRon:100,sellPriceGrossRon:200,sellVatRate:.21,marketplaceCommissionRate:.15,fulfillmentPerSetRon:15,adsReserveRate:.05,returnsReserveRate:.03,otherReserveRate:.02,importVatRecoverable:true,freightEvidenceRef:'quote',dutyEvidenceRef:'taric',sellPriceEvidenceRef:'market'};
...
test('calculates landed economics only with complete verified inputs',()=>{
  const r=calculateLandedEconomics({
    ...
    goodsValueRon:5000,internationalFreightRon:1000,insuranceRon:50,dutyRate:.12,importVatRate:.21,
    ...
    freightEvidenceRef:'verified-freight',dutyEvidenceRef:'verified-taric',sellPriceEvidenceRef:'verified-market'
  });
