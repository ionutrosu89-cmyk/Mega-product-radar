...
import {buildLandedInputFromVerifiedQuote,buildLandedInputFromSupplierPage} from '../scripts/verified-quote-to-landed-input.mjs';

const verifiedQuote={
  verified:true,evidenceStatus:'MANUALLY_VERIFIED_QUOTE',landedCostEligible:true,
  productCanonicalKey:'car-sunglasses-magnetic-visor-holder',supplierName:'Example Supplier',sourceUrl:'https://example.com/item/123',
  manualVerifiedAt:'2026-08-23T12:00:00Z',unitPrice:0.8,currency:'USD',quoteQuantity:100,
  bulkShippingToRomania:45,shippingCurrency:'USD',incoterm:'FOB'
};

const supplierPage={
  supplierPageObserved:true,evidenceStatus:'SUPPLIER_PAGE_OBSERVED',
  productCanonicalKey:'car-sunglasses-magnetic-visor-holder',supplierName:'Example Supplier',sourceUrl:'https://example.com/product/123',
  unitPrice:0.72,currency:'USD',moq:50,productLengthCm:12,productWidthCm:4,productHeightCm:3,actualGrossWeightKg:0.18
};
...
test('unverified public data cannot enter strict verified-quote bridge',()=>{
...
test('exact supplier product page can seed page-backed landed screening without supplier contact',()=>{
  const result=buildLandedInputFromSupplierPage(supplierPage);
  assert.equal(result.ready,true);
