...
function mockFetch(plan='RADAR'){
  return async url=>{
    ...
    if(u.includes('/rest/v1/workspace_members'))return Response.json([{workspace_id:'w1',user_id:'u1',role:'MEMBER'}]);
    if(u.includes('/rest/v1/workspaces'))return Response.json([{id:'w1',name:'Radar',plan,owner_id:'owner'}]);
