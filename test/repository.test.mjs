...
test("radar-data exposes live data and scan state from blobs only to Radar entitlement", async () => {
  ...
  const fetchImpl=async url=>{const u=String(url);if(u.includes('/auth/v1/user'))return Response.json({id:'u1'});if(u.includes('/rest/v1/workspaces'))return Response.json([{id:'w1',plan:'RADAR'}]);return new Response(null,{status:404});};
...
test("public trigger queues the protected background route without exposing its secret", async () => {
  ...
  assert.equal(response.status, 202);assert.match(outbound.url, /^https:\/\/radar\.example\/api\/radar\/scan\?scanId=/);assert.equal(outbound.options.headers["x-radar-secret"], "server-only");assert.equal(JSON.parse(writes.get("scan-status")).status, "queued");assert.equal(JSON.stringify(await response.json()).includes("server-only"), false);
});
...
test("products.json has the frontend's required shape", async () => {
  const products = JSON.parse(await readFile("products.json", "utf8")); const required = ["name", "cat", "chinaMin", "chinaMax", "landed", "sell", "gap", "score", "markets"];assert.ok(Array.isArray(products)); assert.ok(products.length > 0);for (const product of products) { assert.deepEqual(required.filter((field) => !(field in product)), []); for (const field of ["chinaMin", "chinaMax", "landed", "sell", "gap", "score"]) assert.equal(Number.isFinite(product[field]), true, `${product.name}: ${field}`); }
});
