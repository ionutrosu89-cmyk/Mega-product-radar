...
test('landed cost cannot be confirmed without price, rate and quantity',()=>{
  assert.equal(landedCostStatus({canonicalProductId,confirmed:true}).status,'INCOMPLET');
});

test('valid landed cost stays simulated until complete evidence, canonical identity and manual confirmation',()=>{
  ...
  const {canonicalProductId:_,...legacy}=base;
  assert.equal(landedCostStatus({...legacy,confirmationRequested:true}).status,'SIMULAT');
});
