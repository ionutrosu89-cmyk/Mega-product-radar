...
const identityKey=x=>isCanonicalProductId(x.canonicalProductId)?`canonical:${String(x.canonicalProductId).toLowerCase()}`:`legacy:${String(x.productKey||'').toLowerCase()}`;
const keyOf=x=>`${identityKey(x)}::${String(x.supplierName||'').toLowerCase()}`;
let queue=null,candidates=null,rfqText='',activeKey='';

function mergeSeed(seed,stored){const map=new Map((stored||[]).map(x=>[keyOf(x),x]));return seed.map(x=>map.has(keyOf(x))?{...x,...map.get(keyOf(x)),canonicalProductId:x.canonicalProductId||map.get(keyOf(x)).canonicalProductId||null,sourceUrl:x.sourceUrl||map.get(keyOf(x)).sourceUrl}:x);}
...
function quoteUrl(r){const params={product:r.productName,supplier:r.supplierName,platform:r.platform||''};if(isCanonicalProductId(r.canonicalProductId))params.canonicalProductId=String(r.canonicalProductId).toLowerCase();const p=new URLSearchParams(params);return`./supplier-intelligence.html?${p.toString()}`;}
...
function render(){
  ...
  $('#supplierGrid').innerHTML=sla.map(({r,s})=>{
    ...
    const identity=`<div class="meta">Identity: ${isCanonicalProductId(r.canonicalProductId)?'CANONICAL':'PENDING — Quote/Supplier Gate fail-closed'}</div>`;
    let actions='';
