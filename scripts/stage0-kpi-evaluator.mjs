...
const cumulativeSpendConservativeEur=num(budget?.spentUsd);
const criteria={minEnrichmentSuccessPct:50,maxCostPerUsefulEnrichmentEur:.50,maxStage0SpendEur:10};
const checks={allowlistActive:audit?.status==='ACTIVE',pipelineDatasetAvailable:products.length>0,targetSetEvaluable:targetCount>0&&evaluated.every(x=>Boolean(x.title)),enrichmentSuccess:successPct>=criteria.minEnrichmentSuccessPct,costEfficiency:costPerUseful!==null&&costPerUseful<=criteria.maxCostPerUsefulEnrichmentEur,stageBudgetSafe:cumulativeSpendConservativeEur<=criteria.maxStage0SpendEur,noCostAnomaly:num(provider?.stats?.costAnomalyCount)===0};
...
const out={version:'1.1',updatedAt:now,verdict,manualPromotionRequired:true,metrics:{pipelineProducts:products.length,targetCount,usefulEnrichments:usefulCount,enrichmentSuccessPct:successPct,runCostConservativeEur,costPerUsefulEnrichmentEur:costPerUseful,cumulativeTestSpendConservativeEur:cumulativeSpendConservativeEur},criteria,checks,blockers,evaluated,policy:'The 100-candidate Stage 0 universe lives in Supabase and is not inferred from the smaller market-intelligence execution dataset. Stage 1 is never enabled automatically. This evaluator only reports whether the current paid batch meets quality and cost thresholds; commercial TEST/BUY gates remain unchanged.'};
