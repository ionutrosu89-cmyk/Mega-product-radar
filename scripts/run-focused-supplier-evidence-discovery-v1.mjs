...
for(const query of queries){
  ...
  for(const row of rows)if(!candidates.has(row.url))candidates.set(row.url,row);
...
const output={
  ...
  queries,searchesAttempted:queries.length,uniqueCandidateUrls:candidates.size,detailsAttempted:details.length,
