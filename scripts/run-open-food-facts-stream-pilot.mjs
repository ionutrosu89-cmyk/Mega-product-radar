...
const maxRows=Math.max(1,Number(process.env.MPR_OFF_STREAM_MAX_ROWS||48000));
const minRows=Math.max(1,Number(process.env.MPR_OFF_STREAM_MIN_ROWS||40000));
const minAccepted=Math.max(1,Number(process.env.MPR_OFF_STREAM_MIN_ACCEPTED||10000));
const minStrongIdentityProducts=Math.max(1,Number(process.env.MPR_OFF_STREAM_MIN_STRONG_IDENTITIES||10000));
...
const strongCandidates=ingestion.accepted.filter(candidate=>candidate.identityStrength==='STRONG_GTIN');
const reviewSample=buildDeterministicReviewSample(strongCandidates,reviewSampleSize);
const dryRun=evaluateOffTenKDryRun(
  {pilot,ingestion,reviewSample},
  {minAccepted,minStrongIdentityProducts,minReviewSample:reviewSampleSize}
);
const tenKCandidateSet=dryRun.decision==='TEN_K_DRY_RUN_EVIDENCE_READY';

const report={
  schema:'MPR_OFFICIAL_OFF_STREAM_PILOT_RUN_V3',
  ...
  dryRun,
  tenKCandidateSet,
  tenKCandidateDecision:tenKCandidateSet?'TEN_K_STRONG_IDENTITY_DRY_RUN_READY':'HOLD_10K_CANDIDATES',
  ...
  note:enabled?'Official Open Food Facts bulk export was streamed and truncated after the configured projected-row limit. Only checksum-valid STRONG_GTIN candidates may satisfy the 10K dry-run gate. No database write or production scale authorization occurred.':'Remote fetch is disabled by default; local fixture evidence cannot establish a real catalog milestone.'
};
...
await writeFile(`${outDir}/review-sample.json`,JSON.stringify({schema:'MPR_OFF_STRONG_IDENTITY_REVIEW_SAMPLE_V1',count:reviewSample.length,candidates:reviewSample},null,2));
