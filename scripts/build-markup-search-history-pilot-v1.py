...
PRODUCTS=Path(sys.argv[1]); SEARCHES=Path(sys.argv[2]); CURRENT=Path(sys.argv[3]); OUT_ROWS=Path(sys.argv[4]); OUT_RECEIPT=Path(sys.argv[5]); LIMIT=int(sys.argv[6] if len(sys.argv)>6 else 25); OFFSET=int(sys.argv[7] if len(sys.argv)>7 else 0)
...
if LIMIT<1 or LIMIT>9000: raise SystemExit('LIMIT_OUT_OF_RANGE')
if OFFSET<0 or OFFSET+LIMIT>9000: raise SystemExit('WINDOW_OUT_OF_RANGE')
...
rows=[best[a][1] for a in window]
if len(rows)!=LIMIT: raise SystemExit(f'ROWS_INSUFFICIENT:{len(rows)}')
if any(sum(k in r for k in ('price','rating','reviewCount'))<2 for r in rows): raise SystemExit('MIN_TWO_METRICS_BROKEN')
metric_coverage={'price':sum('price' in r for r in rows),'rating':sum('rating' in r for r in rows),'reviews':sum('reviewCount' in r for r in rows),'threeMetrics':sum(sum(k in r for k in ('price','rating','reviewCount'))==3 for r in rows)}
planned=metric_coverage['price']+metric_coverage['rating']+metric_coverage['reviews']
receipt={'schemaVersion':'MPR_MARKUP_SEARCH_HISTORY_PILOT_BUILD_V1','selected9kSha256':selected_digest,'searchesSha256':SEARCHES_SHA,'sourceRows':source_rows,'overlapRows':overlap_rows,'dateMissingRows':date_missing,'eligibleAsinsTwoPlusMetrics':len(eligible),'offset':OFFSET,'pilotRows':len(rows),'observationRowsPlanned':planned,'pilotAsinSha256':hashlib.sha256(('\n'.join(r['externalId'] for r in rows)).encode()).hexdigest(),'metricCoverage':metric_coverage,'policy':{'historicalOnly':True,'freshnessClass':'HISTORICAL_2021_NOT_LIVE','observedAtPrecision':'DAY','verifiedSales':False,'providerSpendEur':0,'paidCallsTriggered':0,'purchaseAuthorized':False,'databaseWrites':0}}
