...
const supplierReplyDir=process.argv[6]||'data/supplier-replies';
...
async function loadSupplierReplies(dir){
  const out=new Map();
  let names=[];
  try{names=await fs.readdir(dir);}catch{return out;}
  for(const name of names.filter(x=>x.endsWith('.json')&&!x.startsWith('_'))){
    try{
      const raw=JSON.parse(await fs.readFile(path.join(dir,name),'utf8'));
      const evidence=normalizeSupplierDirectReplyEvidence(raw);
      if(evidence.externalId)out.set(String(evidence.externalId),evidence);
    }catch{}
  }
  return out;
}
const replyById=await loadSupplierReplies(supplierReplyDir);
...
for(const row of oldRows){
  ...
  const spBase={
    externalId:id,productType:s.productType,primaryFunction:s.primaryFunction,packCount:packEvidence.packCount,
  ...
  const spOverlay=applySupplierReplyMatchOverlay(spBase,replyById.get(id)??null);
  const {externalId:_,replyOverlay,...spInput}=spOverlay;
  ...
  output.push({
    ...
    supplierEvidence:{...spInput,priceUnit:raw.priceUnit,categoryId:raw.categoryId??null,packEvidence,attributeCount:detail.evidence?.coverage?.attributeCount??0,replyOverlay},
    match,
    evidenceClass:replyOverlay?.applied?'POST_DETAIL_PLUS_SUPPLIER_DIRECT_REPLY_EVIDENCE':'POST_DETAIL_FUSED_PUBLIC_EVIDENCE',
    truthPolicy:{amazonExplicitPackLabelIsDirectEvidence:true,amazonMissingPackLabelMeansOne:false,derivedPackCountIsDirectSupplierClaim:false,publicSupplierDetailIsVerifiedQuote:false,marketplacePriceIsRealizedSale:false,supplierReplySimilarProductMayTransfer:false,unknownEqualsZero:false,matchingThresholdRelaxed:false,purchaseAuthorized:false,negotiationIncluded:false}
  });
...
const summary={
  ...
  supplierDetailRows:details.length,supplierDirectReplyCount:replyById.size,supplierReplyOverlayCount:output.filter(x=>x.supplierEvidence?.replyOverlay?.applied).length,
  amazonLiveDimensionFetchOk:output.filter(x=>x.amazonEvidence?.liveDetailFetch?.ok).length,
  ...
  policy:{...SupplierReplyMatchOverlayTruthPolicy,paidCallsTriggered:0,providerSpendUsd:0,matchingThreshold:80,matchingThresholdRelaxed:false,unknownEqualsZero:false,purchaseAuthorized:false,negotiationIncluded:false}
};
