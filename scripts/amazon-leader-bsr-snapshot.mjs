...
async function fetchOne(t){
  ...
  try{
    ...
    return{asin,title:t.title||null,reviewDelta:t.reviewDelta??null,url,statusCode:response.status,htmlBytes:html.length,observedAt:now,ok:response.ok&&parsed.ok,...parsed,error:null};
  }catch(error){return{asin,title:t.title||null,reviewDelta:t.reviewDelta??null,url,statusCode:null,htmlBytes:0,observedAt:now,ok:false,status:'FETCH_ERROR',identityConfirmed:false,entries:[],rankEvidenceCount:0,error:String(error?.message||error)};}
}

const rows=[];
for(let i=0;i<targets.length;i+=4)rows.push(...await Promise.all(targets.slice(i,i+4).map(fetchOne)));
const observations=rows.filter(r=>r.ok&&r.rankEvidenceCount>0).map(r=>({
  ...
  provenance:{statusCode:r.statusCode,htmlBytes:r.htmlBytes,identityConfirmed:r.identityConfirmed,providerSpendEur:0}
}));
const result={
  schemaVersion:'MPR_AMAZON_LEADER_BSR_SNAPSHOT_V1',generatedAt:now,targetCount:targets.length,
  validPageCount:rows.filter(r=>r.ok).length,productsWithExplicitBsr:observations.length,
  explicitBsrEntryCount:observations.reduce((n,o)=>n+o.rankEvidenceCount,0),observations,
  diagnostics:rows.map(r=>({asin:r.asin,statusCode:r.statusCode,htmlBytes:r.htmlBytes,ok:r.ok,status:r.status,identityConfirmed:r.identityConfirmed,rankEvidenceCount:r.rankEvidenceCount,error:r.error})),
  policy:{explicitBsrOnly:true,preserveAllCategoryRanks:true,choosePrimaryRank:false,htmlPositionIsNotRank:true,minimumRankTrendIntervalHours:24,salesEvidenceClass:'NOT_VERIFIED_SALES',verifiedSalesRows:0,providerSpendEur:0,paidCallsTriggered:0,purchaseAuthorized:false}
};
