...
function safeInt(v){if(v===null||v===undefined||v==='')return null;const n=Number(v);return Number.isInteger(n)&&n>=0?n:null;}
...
const output={...data,version:'1.6',updatedAt:data.updatedAt||new Date().toISOString(),maxReviews,maxOrganicPage:maxPage,maxObservedSellers:maxSellers,products,feed,feedCount:feed.length,qualityPostprocess:{version:'1.1',at:new Date().toISOString(),exactListingReviewGate:true,missingReviewIsNotZero:true,categoryRelevanceGate:true,imagePlaceholderFilter:true},policy:'Gate obligatoriu pe listarea sursă: review-uri observate ca număr întreg <=10; lipsa review-urilor rămâne NECUNOSCUT și nu trece filtrul. Poziție organică în maximum pagina 2, rezultat nepromovat când marcajul este detectabil, competiție observată redusă și potrivire cu nișa scanată. Sellerii și competiția RO rămân proxy-uri explicite; câmpurile necunoscute nu sunt inventate.'};
