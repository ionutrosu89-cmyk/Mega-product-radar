...
const state=await read(CAP_FILE,{version:'1.1',startedAt:now,capUsd:CAP_USD,providerEvents:[],keywordAccountedUsd:0});
state.version='1.1';
...
async function keywordSpentSinceStart(){
  ...
  return (Array.isArray(b.events)?b.events:[]).filter(e=>new Date(e.at||0).getTime()>=start).reduce((s,e)=>s+n(e.costUsd),0);
}

if(MODE==='post'){
  const provider=await read(PROVIDER,null);
  const runId=String(provider?.updatedAt||'');
  const reportedCost=n(provider?.stats?.reportedCostUsd);
  const accountedCost=n(provider?.stats?.accountedCostUsd||provider?.stats?.runCostUsd);
  if(runId&&accountedCost>0&&!state.providerEvents.some(e=>e.runId===runId)){
    state.providerEvents.push({runId,at:now,costUsd:accountedCost,reportedCostUsd:reportedCost,costStatus:provider?.stats?.costStatus||'LEGACY',costAnomalyCount:n(provider?.stats?.costAnomalyCount),source:'DATAFORSEO_PROVIDER'});
  }
}
...
state.updatedAt=now;
state.policy=`Hard test budget: provider accountedCostUsd is authoritative. If a paid endpoint reports zero/missing cost, the provider layer books a conservative reserve. Stop new paid DataForSEO cycles when less than $${MAX_NEXT_CYCLE_USD.toFixed(2)} remains, so configured per-cycle spending cannot exceed the $${CAP_USD.toFixed(2)} total cap.`;
