...
const input=String(args.input||'artifacts/amazon-current-refresh-targets.json');
const out=String(args.out||'artifacts/amazon-current-refresh-pilot.json');
...
if(targets.some(x=>!/^B[A-Z0-9]{9}$/.test(String(x.external_id||''))||Number(x.live_observation_count)<1||!x.last_live_observed_at))throw new Error('TARGET_SCOPE_INVALID');
...
if(payload.validObservations<Math.ceil(targets.length*0.6)){console.error('CURRENT_REFRESH_COVERAGE_TOO_LOW');process.exitCode=2;}
