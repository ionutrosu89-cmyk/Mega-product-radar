...
for(const file of files){const prefix=file.match(/^(\d{8}(?:\d{6})?)_/)?.[1];if(!prefix)throw new Error(`INVALID_MIGRATION_NAME:${file}`);const key=file.toLowerCase();if(seen.has(key))duplicates.add(file);seen.add(key);const sql=await readFile(`${dir}/${file}`,'utf8');if(!sql.trim())throw new Error(`EMPTY_MIGRATION:${file}`);}
