...
const result=buildFirstFinalistDecision({candidateAsin:candidate,trendFusion:fusion,romaniaEvidence:romania,importabilityEvidence:importability,supplierEvidence:supplier,economicsEvidence:economics});
fs.mkdirSync(new URL('.',`file://${process.cwd()}/${out}`).pathname,{recursive:true});
fs.writeFileSync(out,JSON.stringify(result,null,2)+'\n');
console.log(JSON.stringify({status:result.status,candidateAsin:result.candidateAsin,gates:result.gates,blockers:result.blockers,supplierSourcingEligible:result.supplierSourcingEligible},null,2));
