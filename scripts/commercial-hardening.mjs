...
await fs.writeFile(OUT,JSON.stringify({version:'1.2',updatedAt:new Date().toISOString(),policy:'Commercial facts are separated from proxies. Supplier sourcing is page-backed: direct product/supplier pages may satisfy sourcing screening without outreach. Sales estimates remain distinct from verified sales. Missing evidence never becomes zero; TEST/BUY still require independent landed economics.',supplierPageRegistryStatus:supplierPageRegistry.status||'UNKNOWN',stats,calibration,items:rows},null,2)+'\n');
