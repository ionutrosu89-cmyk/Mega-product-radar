...
async function load(){
  ...
  try{
    const r=await fetch('/api/commercial/discover',{headers,cache:'no-store'});
    const data=await r.json();
    if(!r.ok||!data.ok)throw new Error(data.error||'Discover API unavailable');
    products=Array.isArray(data.products)?data.products:[];
    discoverFull=Boolean(data.entitlements?.discoverFull);
    radarAccess=Boolean(data.entitlements?.radar);
    planCode=String(data.plan||'FREE');
  }catch{
    products=[];discoverFull=false;radarAccess=false;planCode='FREE';
  }
  document.querySelector('#planName').textContent=planCode;
  const cta=document.querySelector('#upgradeCta');if(cta){cta.textContent=discoverFull?'Discover activ':'Deblochează Discover · €17,90';if(discoverFull)cta.href='account.html';}
  render();
}
