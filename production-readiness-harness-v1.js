...
export function buildProductionReadinessSnapshot(input={},options={}){
  ...
  const finalScale=evaluateScaleGate({manifest:{canonicalCount:Number(input.canonicalCount||0),logicalDuplicateCount:Number(input.logicalDuplicateCount||0)}},{requiredCanonicalCount:1000000,provenanceComplete:input.provenanceComplete===true,restoreVerified:restore.restoreVerified,replayDeterministic:input.replayDeterministic===true,queuesStable:queue.queuesStable,p95Ms:latency.p95Verified?latency.p95Ms:null,p95LimitMs:options.p95LimitMs||1000});
