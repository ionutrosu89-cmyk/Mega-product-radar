...
function credentials(requireBetaAcceptance=false){const email=$('#email').value.trim(),password=$('#password').value;if(!email.includes('@'))throw new Error('Introdu un email valid.');if(password.length<8)throw new Error('Parola trebuie să aibă minimum 8 caractere.');if(requireBetaAcceptance&&!$('#acceptBeta')?.checked)throw new Error('Pentru crearea contului trebuie să confirmi testarea profesională și să accepți documentele beta.');return{email,password};}
