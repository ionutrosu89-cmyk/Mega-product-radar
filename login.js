...
$('#signup').addEventListener('click',async()=>{try{status('Creare cont...');const {email,password}=credentials();const data=await signUp(email,password);status(data.session?'Cont creat. Redirectionare...':'Cont creat. Verifica emailul pentru confirmare.','ok');if(data.session)location.href=destination();}catch(e){status(e.message,'error');}});
