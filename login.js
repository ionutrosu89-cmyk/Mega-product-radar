...
getCurrentSession().then(s=>{if(s)location.href='account.html';});
$('#login').addEventListener('click',async()=>{try{status('Autentificare...');const {email,password}=credentials();await signInWithPassword(email,password);location.href='account.html';}catch(e){status(e.message,'error');}});
$('#signup').addEventListener('click',async()=>{try{status('Creare cont...');const {email,password}=credentials();const data=await signUp(email,password);status(data.session?'Cont creat. Redirectionare...':'Cont creat. Verifica emailul pentru confirmare.','ok');if(data.session)location.href='account.html';}catch(e){status(e.message,'error');}});
