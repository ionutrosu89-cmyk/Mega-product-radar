...
function card(x,rank,extra=''){
  const p=x.testPlan;
  const supplier=x.signals.supplierReady?'STRONG':x.signals.supplierPartial?'PARTIAL':'NEVALIDAT';
  return `<article class="card ${extra}"><div class="row"><div><div class="rank">#${rank} • ${esc(x.category)} • confidence ${esc(x.confidence)}</div><div class="name">${esc(x.name)}</div>${badge(x.decision)}</div><div class="score">${x.score}</div></div><div class="meta">Evidence ${x.evidenceScore}/100 • Profit ${money(x.economics.profit)} • marjă ${x.economics.margin.toFixed(1)}% • ROI ${x.economics.roi.toFixed(0)}% • landed ${money(x.economics.landed)} • sell ${money(x.economics.sell)}</div><div class="signals">LIVE ${x.signals.dataLive?'DA':'NU'} • checks ${x.signals.checks} • extern ${x.signals.foreign} • supplier ${supplier} • review ${esc(x.signals.reviewEvidence||x.signals.reviewSources)} • RO gap ${x.signals.romaniaGap} • trend ${x.signals.trend}</div>${x.blockers.length?`<div class="blockers">Blocaje: ${x.blockers.join(' • ')}</div>`:''}${p?`<div class="plan">Plan test: ${p.units} buc. • investiție ${money(p.investment)} • venit potențial ${money(p.revenue)} • profit potențial ${money(p.profitPotential)}</div>`:''}</article>`;
}
