...
function tabs(){
  ...
  root.addEventListener('click',event=>{const btn=event.target.closest('[data-niche]');if(!btn)return;const next=niches.find(n=>n.id===btn.dataset.niche);if(!next)return;current=next;draw();trackFreeDemand('FREE_NICHE_SELECTED',{nicheId:next.id,nicheLabel:next.label});render();});
...
function card(raw,movement,previousReviewedAt,currentEvidence,currentReviewedAt){
  ...
  const sourceUrl=safeUrl(p.sourceUrl);
...
async function loadLiveNiches(){
  for(const endpoint of ['/api/free/niches','/api/free/top25']){
    try{
      ...
      niches=live.slice(0,25);
      current=niches[0]||null;
      return true;
    }catch{/* Try the compatible endpoint, then fail closed. */}
  }
  return false;
}

const loaded=await loadLiveNiches();
tabs();
installFreeDemandTracking(document);
if(loaded)trackFreeDemand('FREE_TOP25_VIEW',{nicheCount:niches.length,productCount:niches.length*25});
$('#grid').addEventListener('click',event=>{
  ...
  if(opened){
    const metadata={productName:product,nicheId:current.id,nicheLabel:current.label,target:opened.getAttribute('href')||''};
    trackJourneyEvent('PRODUCT_OPENED',{product,...metadata});
    trackFreeDemand(opened.dataset.productAction==='source'?'FREE_SOURCE_OPENED':'FREE_PRODUCT_OPENED',metadata);
  }
  ...
  trackFreeDemand('FREE_DECISION_REACHED',{productName:product,nicheId:current.id,nicheLabel:current.label,decision:decision.dataset.productDecision});
});
if(loaded)render();else{
  $('#nicheTitle').textContent='Top 25 indisponibil temporar';
  $('#nicheText').textContent='Datele licențiate nu au putut fi încărcate. Nu afișăm surse de rezervă cu drepturi neconfirmate.';
  $('#coverage').textContent='0/25';
  $('#trackingStatus').textContent='FAIL-CLOSED · încearcă din nou mai târziu';
}
