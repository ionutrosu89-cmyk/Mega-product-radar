...
const RO_PRODUCT_ALIASES=Object.freeze({
  CASA:['organizator casa','cutie depozitare','cos depozitare','raft depozitare'],
  AUTO:['organizator auto','suport telefon auto','suport pahare auto','suport tetiera'],
  ELECTRONICE:['hub usb','cablu usb','suport laptop','statie incarcare'],
  BEAUTY:['organizator machiaj','pensule machiaj','perie par','geanta cosmetice'],
  PET:['pat animale','bol animale','fantana apa pisici','perie animale'],
  SPORT:['geanta sport','pompa minge','conuri antrenament','suport mingi'],
  COPII:['organizator jucarii','tava activitati copii','geanta pranz copii'],
  BIROU:['organizator birou','suport monitor','suport documente','suport laptop'],
  ORGANIZARE_CASA:['organizator sertar','organizator dulap','depozitare sub pat','organizator pantofi'],
  CURATENIE:['perie curatare','laveta microfibra','rola scame','racleta geam'],
  TELEFON_TECH:['suport telefon','suport tableta','cablu incarcare','organizator cabluri'],
  FITNESS_ACASA:['banda elastica fitness','rola spuma','coarda sarit','bloc yoga'],
  GRADINA_BALCON:['suport plante','clipsuri plante','duza furtun','jardiniera'],
  DIY_SCULE:['organizator scule','suport scule','panou perforat scule','accesoriu bormasina'],
  BABY_ACCESORII:['organizator carucior','organizator scutece','bavetica','salteluta schimbat'],
  AUTO_ACCESORII:['organizator portbagaj','organizator scaun auto','cos gunoi auto','suport auto'],
  HOBBY_CRAFT:['organizator craft','organizator margele','organizator pictura','organizator cusut'],
  PARTY:['decoratiuni petrecere','banner aniversar','topper tort','accesorii baloane'],
  BIROU_ORGANIZARE:['organizator documente','suport dosare','organizator pixuri','raft birou'],
  FASHION_ORGANIZARE:['husa haine','organizator bijuterii','organizator garderoba','organizator genti'],
  CAINI_ACCESORII:['jucarie caine','lesa caine','ham caine','bol caine'],
  PISICI_ACCESORII:['jucarie pisica','litiera pisici','covoras litiera','tunel pisici'],
  LAUNDRY:['cos rufe','sac rufe','uscator rufe pliabil','rola scame haine'],
  COPII_EDUCATIONAL:['jucarie educativa','jucarie montessori','carduri educative','joc sortare copii'],
  CALATORII:['cuburi bagaj','organizator bagaj','eticheta bagaj','perna calatorie','suport pasaport']
});
...
export function classifyCurrentSearchProductSignal(query){
  ...
  for(const [nicheId,aliases] of Object.entries(RO_PRODUCT_ALIASES)){
    const include=aliases.find(term=>contains(text,term));
    if(include&&!matches.some(row=>row.nicheId===nicheId))matches.push({nicheId,matchedTerm:include});
  }
