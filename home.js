...
async function load(){
  ...
  $('#welcomeTitle').textContent=plan==='FREE'?'Începe cu ce se mișcă în piață':plan==='DISCOVER'?'Găsește semnalele care merită urmărite':plan==='RADAR'?'Transformă semnalele în decizii comerciale':'Construiește și execută planul de lansare';
  $('#welcomeText').textContent=plan==='FREE'?'Vezi primele oportunități și completează profilul comercial.':plan==='DISCOVER'?'Folosește Discover pentru a găsi produsele în creștere, apoi treci în Radar când vrei validare pentru România.':plan==='RADAR'?'Concentrează-te pe produsele cu cele mai puține blocaje și nu aloca bani până când landed cost-ul și gate-urile sunt confirmate.':'Folosește shortlist-ul personalizat, bugetul și traseul de execuție pentru testele reale.';
  ...
  const cards=[];
  cards.push(card('1. Discover','Vezi produse în trend, Rising, New și semnale Amazon/TikTok.','discover.html','Deschide Discover','HOME_OPEN_DISCOVER'));
  cards.push(card('2. Radar',rank>=2?'Vezi România Gap, furnizor, landed cost, profit, ROI și verdictul TEST/HOLD.':'Deblochează validarea comercială pentru România.',rank>=2?'commercial-radar.html':'pricing.html?upgrade=RADAR',rank>=2?'Deschide Radar':'Vezi Radar · €29',rank>=2?'HOME_OPEN_RADAR':'UPGRADE_INTENT_RADAR',rank<2));
  cards.push(card('3. Launch',rank>=3?'Vezi shortlist-ul personalizat, bugetul și următorii pași de execuție.':'Deblochează planul complet de lansare și capital.',rank>=3?'commercial-launch.html':'pricing.html?upgrade=LAUNCH',rank>=3?'Deschide Launch':'Vezi Launch · €89',rank>=3?'HOME_OPEN_LAUNCH':'UPGRADE_INTENT_LAUNCH',rank<3));
